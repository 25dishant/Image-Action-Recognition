...
import Loader.metric as metric
...
def split_and_load(batch):
    ...
    for i, data in enumerate(batch):
        new_data = [x for x in zip(data)]
        new_batch.append(new_data)
    return new_batch

def validate(net, val_data,eval_metric):
    ...
    for batch in val_data:
        ...
        for data, label, box in zip(*batch):
            ...
            cls_score = torch.softmax(cls_score)
...
def Train(net, train_data, val_data, eval_metric, args):
    ...
    #      'clip_gradient': 5})
    trainer = {'learning_rate': args.max_lr,
         'wd': args.wd,
         'momentum': args.momentum,
         'clip_gradient': 5}
    ...
    # cosine lr annealing
    lr_annealing = lr_schedule.CosineAnnealingSchedule(min_lr=args.min_lr, max_lr=args.max_lr, cycle_length=args.cycle_len)
    epoch_size = len(train_data)

    # rcnn_cls_loss = mx.gluon.loss.SoftmaxCrossEntropyLoss()
    rcnn_cls_loss = torch.nn.CrossEntropyLoss()
    metrics = [Loss('RCNN_CrossEntropy'), ]
    ...
    #         batch = split_and_load(batch, ctx_list=ctx)
        ...
        if not (epoch + 1) % args.val_interval:
            ...
            map_name, mean_ap = validate(net, val_data, eval_metric)
